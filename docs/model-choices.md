...
We chose MiniLM because the corpus is small, the latency budget is tight, and the 90MB model can be vendored for fully offline operation. It is a 2021 model and newer small embedders (bge-small-en-v1.5, nomic-embed-text) score several MTEB points higher at similar cost; the embedder is one function behind `app/rag/embedder.py`, so swapping it is a one-line change plus a re-index, and we would benchmark exactly that swap before any production use. The summarizer is `claude-opus-4-8` (same configuration as stage 2; the dossier synthesis is comparably heavy). Precedent facts (case_ref, similarity, outcome) are copied from retrieval results after parsing (`backend/app/llm/stages/stage3_adjudication.py`); the model authors only the per-case relevance note, so it cannot fabricate precedents.
